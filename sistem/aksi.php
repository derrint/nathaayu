...
include "modul/function.php";
		...
		require '../escpos-php-master/autoload.php';
...
//echo $sql_simpan;
      ...
      if (cekHargaJualBerubah($simpan['barcode'], $simpan['hargaJual'])) {
...
// siapkan string yang akan dicetak
   ...
   $struk .= str_pad($store_name, 32, " ", STR_PAD_BOTH)."\n".str_pad($header1, 32, " ", STR_PAD_BOTH)."\n";
   $struk .= str_pad($_SESSION[uname]." : ".date("d-m-Y H:i")." #$NomorStruk", 32, " ", STR_PAD_BOTH)."\n";
...
//print_r($_SESSION);
      ...
      $struk .= str_pad("{$customer['namaCustomer']} : {$customer['nomor_kartu']}", 32, " ", STR_PAD_BOTH)."\n";
   }
   $struk .= "--------------------------------\n";
...
// Jika ada diskon customer dipisah tampilannya di struk
         ...
         $diskon = str_pad($textDiskon, 32, ' ', STR_PAD_LEFT)."\n";
      }
...
//};
      ...
      $struk .= str_pad($tempHarga, 32, ' ', STR_PAD_LEFT)."\n";
      $struk .= $diskon;
   }
   ...
   $struk .= $diskonHargaPerBarangTotal > 0 && $diskonCustomer > 0 ? str_pad($textTotalPotongan, 20, ' ', STR_PAD_LEFT)." \n" : '';
   $struk .= $diskonCustomer > 0 ? str_pad($textDiskonCustomer, 20, ' ', STR_PAD_LEFT)."\n" : '';
   $struk .= str_pad($textTotal, 20, ' ', STR_PAD_LEFT)."\n";
   $struk .= str_pad($textDibayar, 20, ' ', STR_PAD_LEFT)." \n";
   $struk .= str_pad($textKembali, 20, ' ', STR_PAD_LEFT)." \n";
   $struk .= $diskonHargaPerBarangTotal > 0 ? str_pad($textAndaHemat, 20, ' ', STR_PAD_LEFT)."\n" : '';
   $struk .= "--------------------------------\n";
   $struk .= str_pad($footer1, 32, " ", STR_PAD_BOTH)."\n".str_pad($footer2, 32, " ", STR_PAD_BOTH)."\n\n";
   ...
   if ($jenis_printer === 'rlpr' && $potongKertas) {
	...
	//----------------- ESCPOS ------------------
		...
		try {
			...
			//$connector = null;
			$connector = new WindowsPrintConnector("eppos");
			/* Print a "Hello world" receipt" */
			$printer = new Printer($connector);
			//$printer -> text("Hello World!\n ini adalah pesan dari kerang ajaib \n\n\n\n ------ \n\n");
			$printer -> text($struk);
			$printer -> cut();
...
elseif ($module == 'penjualan_barang' AND $act == 'batal') {
   ...
   if ($_SESSION['hakAdmin'] || $transferahad) {
      ...
      header('location:modul/js_jual_barang.php?act=caricustomer&bg=1');
   }
}
...
elseif ($module === 'diskon' && $act === "getbarcodeinfo") {
   ...
   if (isset($_GET['term'])) {
      ...
      echo $term;
      $sql = "SELECT barcode, namaBarang FROM barang where namaBarang like '%{$namaBarang}%' ORDER BY IFNULL(NULLIF(nonAktif,'0'),0), namaBarang ASC";
      $hasil = mysql_query($sql);
      $barangs = array();
...
} elseif ($module == 'penjualan_barang' AND $act == 'cek_harga') {
   $return = array(
       'sukses' => false
   );
