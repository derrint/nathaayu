...
function tambahBarangJual($barcode, $jumBarang, $hargaBarang) {
   ...
   $qStokBarang = mysql_query("SELECT jumBarang FROM barang WHERE barcode = '$barcode'");
   $rStokBarang = mysql_fetch_array($qStokBarang);
   $stokBarangNow = $rStokBarang['jumBarang'];


   if ($dataAda != 0 and $jumBarang <= $stokBarangNow ) {
      $jumlah = 0;
//      if($jumBarang==0){
      // quantity can not be 0 (zero) or less than that
      if ($jumBarang < 1) {
...
function cetakStruk($nomorStruk, $strukRetur = false) {
   ...
   $struk = str_pad($store_name, 20, " ", STR_PAD_BOTH)."\n".str_pad($header1, 20, " ", STR_PAD_BOTH)."\n"
           .str_pad($namaKasir." : ".$tglTransaksi." #$nomorStruk", 20, " ", STR_PAD_BOTH)." \n";
...
//					  " = " . number_format(($x[hargaJual] * $x[jumBarang]), 0, ',', '.') . "\n";
   ...
   $struk .= "--------------------------------\n";
   $struk .= str_pad($footer1, 20, " ", STR_PAD_BOTH)."\n".str_pad($footer2, 20, " ", STR_PAD_BOTH)."\n\n";
   return $struk;
}
